...
class BagelsGame:
	...
	def prompt(self):
		...
		return 'You have ' + str(self._tries) + ' ' + chances + ' to guess my 3-digit number.'
	...
	def evaluate(self, guess):
		...
		if guess_digits == self._secret_number:
			return hint

		for i in range(len(guess_digits)):
			if guess_digits[i] == self._secret_number[i]:
				hint.append('Fermi')
			elif guess_digits[i] in self._secret_number:
				hint.append('Pico')
